...
import { KrpanCraneInformationComponent } from "../krpan-crane-information/krpan-crane-information.component";
...
import { KrpanCraneConfigService } from '../../services/krpan-crane-config.service';
...
import { KrpanCraneCalculatorHintsComponent } from "../krpan-crane-calculator-hints/krpan-crane-calculator-hints.component";
...
@Component({
    selector: 'app-krpan-crane',
    standalone: true,
    templateUrl: './krpan-crane.component.html',
    styleUrl: './krpan-crane.component.css',
    imports: [FormsModule, InputSwitchModule, ReactiveFormsModule, AccordionModule, RadioButtonModule, CheckboxModule, NavigationComponent, FooterComponent, KrpanCraneInformationComponent, CommonModule, ListboxModule, AccessoryItemComponent, FormatPricePipe, FrameTypesDialogComponent, ControlBlocksDialogComponent, RotatorsDialogComponent, GrapplesDialogComponent, WinchesDialogComponent, ProtectionSleevesDialogComponent, ElectricalFloatingDialogComponent, ValveBlockDialogComponent, DampingsDialogComponent, CraneLightDialogComponent, OperatorSeatDialogComponent, CraneOilcoolerDialogComponent, RotatorBrakesDialogComponent, JoystickHolderDialogComponent, HoseGuardsDialogComponent, TurningCounterPlateDialogComponent, SupportCounterPlateDialogComponent, BoomguardDialogComponent, CoverDialogComponent, WoodcontrolDialogComponent, LinkageDialogComponent, KrpanTrailerComponent, KrpanTrailerCardsComponent, KrpanCraneCalculatorHintsComponent]
})
export class KrpanCraneComponent implements OnInit, OnDestroy {
    crane!: KrpanCrane
    ...
    originalShipping: ConfigurationItem | undefined = undefined;
      ...
      removeGrapple(index: number) {
        this.krpanService._cranePrice.update(cranePrice => cranePrice - this.originalGrapplePrices[index])
        this.krpanService.selectedGrapples.splice(index, 1)
    ...
    constructor(readonly krpanService: KrpanService,
        readonly krpanCraneConfigService: KrpanCraneConfigService,
    ...
    ngOnInit(): void {
      ...
      this.krpanService.getCrane(this.id).pipe().subscribe((response) => {
        if(!this.fromTrailer){
          ...
          this.krpanService._selectedCrane.next(response);
        }
        
        this.crane = response as KrpanCrane;
      ...
      if(this.fromTrailer){
        this.krpanService.selectedCrane$
      ...
      this.krpanService.deleteCrane$.subscribe(() => {  
        this.delete();
      })
    }
    ...
    navigateToTrailer(trailerId: number){
      const url = `/calculator/krpan/trailers/${trailerId}`;
      window.open(url, '_blank');
      }

    loadCraneConfigurations(id: number){ 
        this.loadingService.enableLoader();
        const controlBlocks$ = this.krpanCraneConfigService.getControlBlocks(id);
        const frameTypes$ = this.krpanCraneConfigService.getFrameTypes(id);
        const rotators$ = this.krpanCraneConfigService.getRotators(id);
        const grapples$ = this.krpanCraneConfigService.getGrapples(id);
        const winches$ = this.krpanCraneConfigService.getWinches(id);
        const protectionSleeves$ = this.krpanCraneConfigService.getProtectionSleeves(id);
        const electricalFloating$ = this.krpanCraneConfigService.getElectricalFloating(id);
        const valveBlock$ = this.krpanCraneConfigService.getValveBlock(id);
        const dampings$ = this.krpanCraneConfigService.getDampings(id);
        const light$ = this.krpanCraneConfigService.getLight(id);
        const operatorSeat$ = this.krpanCraneConfigService.getOperatorSeat(id);
        const oilCooler$ = this.krpanCraneConfigService.getOilCooler(id);
        const rotatorBrakes$ = this.krpanCraneConfigService.getRotatorBrakes(id);
        const joystickHolder$ = this.krpanCraneConfigService.getJoystickHolder(id);
        const hoseguards$ = this.krpanCraneConfigService.getHoseGuards(id);
        const turningDeviceCounterPlate$ = this.krpanCraneConfigService.getTurningDeviceCounterPlate(id);
        const supportLegCounterPlate$ = this.krpanCraneConfigService.getSupportLegCounterPlate(id);
        const boomGuard$ = this.krpanCraneConfigService.getBoomGuard(id);
        const cover$ = this.krpanCraneConfigService.getCover(id);
        const woodControl$ = this.krpanCraneConfigService.getWoodControl(id);
        const linkage$ = this.krpanCraneConfigService.getLinkage(id);
        const craneShipping$ = this.krpanCraneConfigService.getShipping(id);
        ...
        request.subscribe(([controlBlocks, frameTypes, rotators, grapples, winches, 
          ...
          woodControl, linkage, craneShipping]) => {
            ...
            if (craneShipping){
              ...
              this.krpanService.selectedCraneShipping.set(craneShipping);
              this.krpanService._cranePrice.set(Number(craneShipping.price));
            }

            this.initializeFormGroup();
            this.craneSelected = true;
            this.krpanService._cranePrice.update(cranePrice => cranePrice + Number(this.crane.price));
        }).add(() => {
          this.loadingService.disableLoader();
          this.krpanService._craneSelected.next(true);
        })
    } 
    ...
    handleControlBlockChange(event: ListboxChangeEvent) {
      ...
      const current = this.krpanService._cranePrice();
    
      if (previousValue !== nextValue) {
        const newPrice = current - previousValue + Number(nextValue);
        this.krpanService._cranePrice.set(newPrice);
      }
      ...
      if (event.value) {
        ...
        this.krpanService.selectedControlBlock.set(event.value)
        ...
        this.krpanService.selectedControlBlock.set(undefined)
    
        updatedFrameTypes = this.updateFrameTypesToEnabled();
        updatedWinches = this.updateWinchesForControlBlock();
        this.setElectricalFloatingDefault();
        this.krpanService.selectedElectricalFloating.set(undefined)
        this.setValveBlockDefault();
        this.krpanService.selectedValveBlock.set(undefined)
        this.setOperatorSeatDefault();
        this.krpanService.selectedOperatorSeat.set(undefined)
        this.setJoystickHolderDefault();
        this.krpanService.selectedJoystickHolder.set(undefined)
        this.setCoverDefault();
        this.krpanService.selectedCover.set(undefined)
    
        // winches
        if (this.originalWinch && (this.originalWinch.id === 2 || this.originalWinch.id === 3)) {
          this.setWinchToDefault();
          this.krpanService.selectedWinch.set(undefined)
        }
      }
    ...
    handleFrameTypeChange(event: ListboxChangeEvent) {
      ...
      if (event.value) {
        ...
        this.krpanService.selectedFrameType.set(event.value)
        ...
        this.krpanService.selectedFrameType.set(undefined);
        updatedControlBlocks = this.updateControlBlocksToEnabled();
        updatedHoseGuards = this.updateHoseGuardsToDisabled();
        this.setHoseGuardToDefault();
        this.setLinkageDefault();
    ...
    handleRotatorChange(event: ListboxChangeEvent) {
      ...
      if (event.value) {
          ...
          this.krpanService.selectedRotator.set(event.value)
          if (event.value.id === parseInt("2")){
            ...
            this.krpanService.selectedGrapple.set(undefined);
            this.setGrapplesToDefault();
            this.krpanService.selectedGrapples = [];
          } else {
            this.updateGrapplesAvailability();
          }
      } else {
        this.originalRotator = undefined;
        this.krpanService.selectedRotator.set(undefined)
    ...
    setGrappleToDefault(){
      if(this.originalGrapple && this.originalGrapple.price){
        this.krpanService._cranePrice.update(value => value - Number(this.originalGrapple?.price))
    ...
    setGrapplesToDefault(){
      this.grappleListBoxes.forEach((grappleListBox, index) => {
        if(grappleListBox.value && grappleListBox.value.id === 2){
          this.krpanService._cranePrice.update(value => value - Number(this.originalGrapples[index]?.price))
          ...
          this.krpanService.selectedGrapples[index] = undefined;
        }
      });
    }
    ...
    setHoseGuardToDefault(){
      if(this.originalHoseGuard && this.originalHoseGuard.price){
        this.krpanService._cranePrice.update(value => value - Number(this.originalHoseGuard?.price))
        this.hoseGuardListBox.writeValue(undefined);
        this.originalHoseGuard = undefined;
        this.krpanService.selectedHoseGuard.set(undefined);
    ...
    handleMultipleGrappleChange(event: ListboxChangeEvent, index: number) {
      ...
      if (event.value){
        ...
        this.krpanService.selectedGrapples[index] = event.value
      } else {
        this.originalGrapples[index] = undefined;
        this.krpanService.selectedGrapples[index] = undefined
      }
    ...
    handleGrappleChange(event: ListboxChangeEvent) {
      ...
      if (event.value){
        ...
        this.krpanService.selectedGrapple.set(event.value)
      } else {
        this.originalGrapple = undefined;
        this.krpanService.selectedGrapple.set(undefined)
      }
    ...
    handleWinchChange(event: ListboxChangeEvent) {
      ...
      if (event.value){
        ...
        this.krpanService.selectedWinch.set(event.value)
      } else {
    ...
    setWinchToDefault(){
      if(this.originalWinch && this.originalWinch.price){
        this.krpanService._cranePrice.update(value => value - Number(this.originalWinch?.price))
    ...
    onProtectionSleevesChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedProtectionSleeves.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
        const newPrice = current - this.originalProtectionSleevesPrice;
        this.krpanService._cranePrice.set(newPrice);
        this.originalProtectionSleeves = undefined;
        this.krpanService.selectedProtectionSleeves.set(undefined)
      }
    }
    ...
    onElectricalFloatingChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedElectricalFloating.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
    ...
    setElectricalFloatingDefault(){
      if(this.originalElectricalFloating && this.originalElectricalFloating.price){
        this.krpanService._cranePrice.update(value => value - Number(this.originalElectricalFloating?.price))
    ...
    onValveBlockChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedValveBlock.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
    ...
    setValveBlockDefault(){
      if(this.originalValveBlock && this.originalValveBlock.price){ 
        this.krpanService._cranePrice.update(value => value - Number(this.originalValveBlock?.price))
    ...
    handleDampingChange(event: ListboxChangeEvent) {
      ...
      if (event.value){
        ...
        this.krpanService.selectedDamping.set(event.value);
      } else {
        this.originalDamping = undefined;
        this.krpanService.selectedDamping.set(undefined);
      }
    }
    ...
    onLightChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedCraneLight.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
        const newPrice = current - this.originalLightPrice;
        this.krpanService._cranePrice.set(newPrice);
        this.originalLight = undefined;
        this.krpanService.selectedCraneLight.set(undefined)
      }
    }
    ...
    onOperatorSeatChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedOperatorSeat.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
    ...
    setOperatorSeatDefault(){
      if(this.originalOperatorSeat && this.originalOperatorSeat.price){
        this.krpanService._cranePrice.update(value => value - Number(this.originalOperatorSeat?.price))
    ...
    onOilCoolerChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedCraneOilCooler.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
        const newPrice = current - this.originalOilCoolerPrice;
        this.krpanService._cranePrice.set(newPrice);
        this.originalOilCooler = undefined;
        this.krpanService.selectedCraneOilCooler.set(undefined)
      }
    }
    ...
    handleRotatorBrakeChange(event: ListboxChangeEvent) {
      ...
      if (event.value){
        ...
        this.krpanService.selectedRotatorBrake.set(event.value)
      } else {
        this.originalRotatorBrake = undefined;
        this.krpanService.selectedRotatorBrake.set(undefined);
      }
    }
    ...
    onJoystickHolderChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedJoystickHolder.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
    ...
    setJoystickHolderDefault(){
      if(this.originalJoystickHolder && this.originalJoystickHolder.price){ 
        this.krpanService._cranePrice.update(value => value - Number(this.originalJoystickHolder?.price))
    ...
    handleHoseGuardChange(event: ListboxChangeEvent) {
      ...
      if (event.value){
        ...
        this.krpanService.selectedHoseGuard.set(event.value)
      } else {
        this.originalHoseGuard = undefined;
        this.krpanService.selectedHoseGuard.set(undefined)
      }
    ...
    onTurningDeviceCounterPlateChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedTurningDeviceCounterPlate.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
        const newPrice = current - this.originalTurningDeviceCounterPlatePrice;
        this.krpanService._cranePrice.set(newPrice);
        this.originalTurningDeviceCounterPlate = undefined;
        this.krpanService.selectedTurningDeviceCounterPlate.set(undefined)
      }
    }
    ...
    onSupportLegCounterPlateChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedSupportLegCounterPlate.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
        const newPrice = current - this.originalSupportLegCounterPlatePrice;
        this.krpanService._cranePrice.set(newPrice);
        this.originalSupportLegCounterPlate = undefined;
        this.krpanService.selectedSupportLegCounterPlate.set(undefined)
      }
    }
    ...
    onBoomGuardChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedBoomGuard.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
        const newPrice = current - this.originalBoomguardPrice;
        this.krpanService._cranePrice.set(newPrice);
        this.originalBoomguard = undefined;
        this.krpanService.selectedBoomGuard.set(undefined)
      }
    }
    ...
    onCoverChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedCover.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
    ...
    setCoverDefault(){
      if(this.originalCover && this.originalCover.price){ 
        this.krpanService._cranePrice.update(value => value - Number(this.originalCover?.price))
    ...
    onWoodControlChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedWoodControl.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
        const newPrice = current - this.originalWoodControlPrice;
        this.krpanService._cranePrice.set(newPrice);
        this.originalWoodControl = undefined;
        this.krpanService.selectedWoodControl.set(undefined)
      }
    }
    ...
    onLinkageChange(event: CheckboxChangeEvent){
      if (event.checked.length > 0) {
        ...
        this.krpanService.selectedLinkage.set(event.checked[0])
      } else {
        const current = this.krpanService._cranePrice();
        const newPrice = current - this.originalLinkagePrice;
        this.krpanService._cranePrice.set(newPrice);
        this.originalLinkage = undefined;
        this.krpanService.selectedLinkage.set(undefined)
      }
    }

    setLinkageDefault(){
      if(this.originalLinkage && this.originalLinkage.price){ 
        this.krpanService._cranePrice.update(value => value - Number(this.originalLinkage?.price))
    ...
    selectTrailer(trailer: KrpanTrailerOverview){
      this.krpanService._selectedTrailer.next(trailer);
    }
